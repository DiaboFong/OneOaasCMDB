...
(function () {
  ...
  angular.module('bsTable', [])
    ...
    var TRASH_SELECTOR = 'button[name="trash"]';
    var FILTER_SELECTOR = '#searchs';
    var SETTINGS_SELECTOR='button[name="settings"]';
    ...
    function getBsTableByClass(className) {
        return $(className);
      }
    ...
    $(document)
      ...
      .on('search.bs.table', CONTAINER_SELECTOR+' table', function (evt, searchText) {
        ...
        bsTable.$s.$applyAsync(function () {
          ...
          state.query(state.searchText);
        });
      })
      .on('search.bs.table', FILTER_SELECTOR, function (evt, searchText) {
          var bsTable = getBsTableByClass(CONTAINER_SELECTOR);
          if (!bsTable) return;
          var state = bsTable.$s.bsTableControl.state;
          bsTable.$s.$applyAsync(function () {
            state.searchText = searchText;
      ...
      .on('click', TRASH_SELECTOR, function (evt) {
          var bsTable = getBsTable(evt.target);
          if (!bsTable) return;
          var state = bsTable.$s.bsTableControl.state;
          bsTable.$s.$applyAsync(function () {
      ...
      .on('click', SETTINGS_SELECTOR, function (evt) {
          var bsTable = getBsTable(evt.target);
          if (!bsTable) return;
          var state = bsTable.$s.bsTableControl.state;
          bsTable.$s.$applyAsync(function () {
      ...
      .on('refresh.bs.table',CONTAINER_SELECTOR+' table', function (evt) {
          var bsTable = getBsTable(evt.target);
          if (!bsTable) return;
          var state = bsTable.$s.bsTableControl.state;
          bsTable.$s.$applyAsync(function () {
      ...
      .on('check.bs.table uncheck.bs.table ' + 'check-all.bs.table uncheck-all.bs.table', function (evt) {
        var bsTable = getBsTable(evt.target);
        if (!bsTable) return;
        var state = bsTable.$s.bsTableControl.state;
        if(bsTable.$el.bootstrapTable('getSelections').length>0){
          $(state.removeSelector).attr('disabled',false);
        }else{
          $(state.removeSelector).attr('disabled',true);
        }
        state.selections = $.map(bsTable.$el.bootstrapTable('getSelections'), function (row) {
          return row;
        });
      })
      return {
        ...
        link: function ($s, $el) {
          ...
          $s.$watch('bsTableControl.state', function (state) {
            //增加触发，本地搜索功能
            if ('searchFilter' in state) $el.bootstrapTable('resetSearch', state.searchText);
